...
The main window contains these top-level tabs:

\begin{itemize}
    \item \ui{Project Files}
    \item \ui{Auto Detection}
    \item \ui{Manual Review}
    \item \ui{Classification}
    \item \ui{Alignment}
\end{itemize}
...
The \ui{Crop} tab selects the region used by later processing. You can process the full image or a rectangular crop.
...
\begin{enumerate}
    ...
    \item The app saves edited events, event pixels, visible IDs, settings, and project file paths.
...
Classification scores the currently reviewed events against crystallographic trace vectors generated in \ui{Auto Detection > Alignment}.
...
The score is an angle-based agreement score. A higher score means the event direction is closer to one of the selected crystallographic trace vectors.
...
\begin{itemize}
    \item Grain triple points.
    \item Twin tips.
    \item Twin-twin contacts.
    \item Twin-grain-boundary contacts.
\end{itemize}
